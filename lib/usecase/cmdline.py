...
For fuller documentation refer to the online docs at http://www.texttest.org"""
    ...
    parser.add_option("-m", "--mapfiles", default=os.path.join(scriptengine.ScriptEngine.usecaseHome, "ui_map.conf"),
...
def check_python_version():
    ...
    if (major, minor) < (reqMajor, reqMinor):
        ...
        raise definitions.UseCaseScriptError, "PyUseCase " + definitions.__version__ + " requires at least Python " + \
...
def main(install_root):
    ...
    try:
        ...
        import usecase
        usecase.scriptEngine = create_script_engine(options, install_root)
        if not usecase.scriptEngine.run(options, args):
            parser.print_help()
