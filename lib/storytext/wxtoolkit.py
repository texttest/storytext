...
class WidgetAdapter(guishared.WidgetAdapter):
    ...
    def isAutoGenerated(self, name):
        ...
        x = name.lower().replace("ctrl", "") in [ cls.__name__.lower(
                            ).replace("ctrl", "") for cls in baseclasses ]
        return x or name == "check"
...
class CheckBoxEvent(SignalEvent):
    ...
    def generate(self, argumentString):
        if self.widget.Is3State():
            self.changeMethod(int(argumentString))
        else:
            self.changeMethod(argumentString == "checked")
        id = self.widget.GetId()
    ...
    def outputForScript(self, *args):
        if self.widget.Is3State():
            text = str(self.widget.Get3StateValue())
        else:
            text = "checked" if self.widget.GetValue() else "unchecked"
