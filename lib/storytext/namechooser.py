...
class UseCaseNameChooser:
    ...
    def __init__(self, fileName, interface, mapFiles, f):

        print >>f, "  Entered UseCaseNameChooser.__init__"; f.flush()
        print >>f, "    fileName =", fileName; f.flush()
        print >>f, "    interface =", interface; f.flush()
        print >>f, "    mapFiles =", mapFiles; f.flush()

        self.fileName = fileName
        self.interface = interface
        print >>f, "    Point 1"; f.flush()
        self.uiMapFileHandler = UIMapFileHandler(mapFiles)
        print >>f, "    Point 2"; f.flush()
        self.scriptEngine = gtktoolkit.ScriptEngine(uiMapFiles=[])
        print >>f, "    Point 3"; f.flush()
        self.allEntries = OrderedDict()
        print >>f, "  Exiting UseCaseNameChooser.__init__"; f.flush()

    def collectNames(self, f):
        print >>f, "    Entered collectNames"; f.flush()
        commands = [ line.strip() for line in open(self.fileName) ]
        print >>f, "      Point 4"; f.flush()
        autoGenerated = self.getAutoGenerated(commands)
        print >>f, "      Point 5"; f.flush()
        if len(autoGenerated) == 0:
            print >>f, "    Exiting collectNames because autoGenerated is empty"; f.flush()
            return

        autoGeneratedInfo = self.parseAutoGenerated(autoGenerated)
        print >>f, "      Point 6"; f.flush()
        dialog = self.createDialog(autoGeneratedInfo, commands)
        print >>f, "      Point 7"; f.flush()
# When StoryText is run under Wing, the next call dies after flashing the 
# dialog on the screen briefly
        response = dialog.run()
        if response == gtk.RESPONSE_ACCEPT:
            print >>f, "      Point 8"; f.flush()
            newNames = [ entry.get_text() for entry in self.allEntries.values() ]
            print >>f, "      Point 9"; f.flush()
            dialog.destroy()
            print >>f, "      Point 10"; f.flush()
            self.replaceInFile(self.fileName, zip(autoGenerated, newNames))
            print >>f, "      Point 11"; f.flush()
            toStore = zip(autoGeneratedInfo, newNames)
            print >>f, "      Point 12"; f.flush()
            ...
            print >>f, "      Point 13"; f.flush()
            self.uiMapFileHandler.write()
        else:
            print >>f, "      Point 14"; f.flush()
            ...
            print >>f, "      Point 15"; f.flush()
            dialog.destroy()
        print >>f, "    Exiting collectNames normally"; f.flush()
    ...
    def createDialog(self, autoGenerated, commands):
        ...
        dialog.set_default_size(-1, int(gtk.gdk.screen_height() * 0.6))
    ...
    def createPreview(self, commands):
        ...
        frame.add(self.addScrollBar(view))
        return frame
...
def main():
    ...
    f = open('/home/roger/opensourcecode/texttest/chooser2.out','w')

    print >>f, "Entered main of namechooser.py"; f.flush()
...
#    import pdb; pdb.set_trace()
    ...
    print >>f, "  options =", options; f.flush()
    print >>f, "  args =", args; f.flush()
    print >>f, "  os.path.isfile(options.record) =", os.path.isfile(options.record); f.flush()

    if os.path.isfile(options.record):
        uiMapFiles = options.mapfiles.split(",")
        level = eval("logging." + options.loglevel.upper())
        logging.basicConfig(level=level, stream=sys.stdout, format="%(message)s")
        chooser = UseCaseNameChooser(options.record, options.interface, uiMapFiles, f)
        chooser.collectNames(f)
    else:
        print >>f, "  ERROR: os.path.isfile fails on", options.record; f.flush()

    print >>f, "Exiting main of namechooser.py"; f.flush()
    f.close()
