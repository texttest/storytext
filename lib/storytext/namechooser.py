...
class UseCaseNameChooser:
    ...
    def __init__(self, fileName, interface, mapFiles):
        ...
        self.shortcuts = self.scriptEngine.getShortcuts()
    ...
    def findShortcutSteps(self, command):
        for shortcut in self.shortcuts:
            if shortcut.getShortcutName() == command:
    ...
    def addCommandToModel(self, command, model, rootIter=None):
        steps = self.findShortcutSteps(command)
        if steps:
            text = "<b>" + command + "</b>"
            iter = self.addText(model, rootIter, text)
            for step in steps:
                self.addCommandToModel(step, model, iter)
        else:
