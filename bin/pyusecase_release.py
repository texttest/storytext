...
# pyusecase_release.py : makes a release from Bazaar source control. Can do so from a local setup using the
# environment variable BZRROOT (modelled on the layout at Jeppesen), or directly from Launchpad.

# Usage pyusecase_release.py [ -v <release_name> ] [ -t <tag> ] [ -x ] [ -d <working_dir> ]
...
# in both the source and the tests
...
def createSource(reldir):
    versionFile = os.path.join(reldir, "source", "lib", "usecase", "definitions.py")
