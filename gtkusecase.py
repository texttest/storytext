...
class UIMap:
    ...
    def autoInstrument(self, eventName, signature, widget, widgetType):
        ...
        if len(parts) > 1:
            ...
            subParts = argumentParseData.rsplit(".", 1)
            if len(subParts) > 1 and subParts[1] in [ "true", "false"]:
                argumentParseData = subParts[0]
                relevantState = subParts[1]
        ...
        if self.widgetHasSignal(widget, signalName):
            ...
            if component:
                ...
                if relevantState is not None:
                    parseData = (widget, relevantState)
                else:
                    parseData = widget
                self.scriptEngine.monitorSignal(eventName, signalName, component, parseData, autoGenerated=True)
                return signature
...
class ScriptEngine(usecase.ScriptEngine):
    ...
    def registerCellToggleButton(self, renderer, checkDescription, parentTreeView, uncheckDescription="", *args, **kwargs):
        if self.active():
            checkChangeName = self.standardName(checkDescription)
            indexer = self.getTreeViewIndexer(parentTreeView, *args, **kwargs)
...
#private
    ...
    def getEventClassArgs(self, widget, argumentParseData):
        ...
        elif type(argumentParseData) == types.TupleType:
            # from CellRenderers etc.
            treeview, relevantState = argumentParseData
            indexer = self.getIndexerFromParseData(treeview, None)
            return (treeview, widget, indexer, relevantState)
        else:
