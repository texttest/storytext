...
class CellToggleEvent(TreeViewEvent):
    signalName = "toggled"
    def __init__(self, name, widget, indexer, cellRenderer):
...
class UIMap:
    ...
    def autoInstrument(self, eventName, signature, widget, widgetType):
        ...
        if self.widgetHasSignal(widget, signalName):
            ...
            component = self.findSubComponent(widget, widgetType, argumentParseData)
            if component and self.widgetHasSignal(component, signalName):
...
class ScriptEngine(usecase.ScriptEngine):
    ...
    def registerCellToggleButton(self, button, description, parentTreeView, *args, **kwargs):
        if self.active():
            ...
            event = CellToggleEvent(eventName, parentTreeView, 
                                    self.getTreeViewIndexer(parentTreeView, *args, **kwargs), button)
            self._addEventToScripts(event)
...
#private
    ...
    def getEventClassArgs(self, widget, argumentParseData):
        ...
        elif isinstance(argumentParseData, gtk.TreeView): # from CellRenderers etc.
            indexer = self.getIndexerFromParseData(argumentParseData, None)
            return (argumentParseData, indexer, widget)
        else:
