...
import usecase, gtklogger, gtk, gobject, os, re, logging
...
class UIMap:
    ...
    def autoInstrument(self, eventName, signalName, widget, widgetType):
        if self.widgetHasSignal(widget, signalName):
            self.logger.debug("Monitor " + eventName + ", " + signalName + ", " + widgetType)
            self.scriptEngine.monitorSignal(eventName, signalName, widget, autoGenerated=True)

    def monitor(self, widget):
        widgetType = widget.__class__.__name__
        if widgetType not in self.ignoreWidgetTypes:
            section = self.findSection(widget, widgetType)
            if section:
                for signalName, eventName in self.parser.items(section):
                    if eventName not in self.storedEvents:
                        self.autoInstrument(eventName, signalName, widget, widgetType)
...
class ScriptEngine(usecase.ScriptEngine):
    ...
    def monitorSignal(self, eventName, signalName, widget, argumentParseData=None, method=None, autoGenerated=False):
        if self.active():
