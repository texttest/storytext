...
class UIMap:
        ...
        if section:
            for signature, eventName in self.parser.items(section):
                if eventName in self.storedEvents:
                    signalsInstrumented.add(signature.split(".")[0])
                else:
                    currSignal = self.autoInstrument(eventName, signature, widget, widgetType)
                    if currSignal:
                        signalsInstrumented.add(currSignal)
                        autoInstrumented = True
        return signalsInstrumented, autoInstrumented

    def findSupportedSignals(self, widget):
        eventClasses = self.scriptEngine.findEventClassesFor(widget)
        return set((eventClass.getAssociatedSignal(widget) for eventClass in eventClasses))

    def monitor(self, widget):
        signalsInstrumented, autoInstrumented = self.instrumentFromMapFile(widget)
        if self.scriptEngine.recorderActive():
            ...
            for signalName in signalsSupported.difference(signalsInstrumented):
                widgetType = widget.__class__.__name__
                autoEventName = "Auto." + widgetType + "." + signalName + ".'" + self.getSectionName(widget) + "'"
                self.autoInstrument(autoEventName, signalName, widget, widgetType)
    ...
    def monitorNewWindows(self):
        for window in gtk.window_list_toplevels():
            if window.get_name() != "Name Entry Window":
...
#                         RowCollapseEvent, RowRightClickEvent ]
    ...
    def replaceAutoRecording(self, testMode):
        autoGenerated = []
        recordScripts = []
        for script in self.recorder.scripts:
            commands = script.getRecordedCommands()
            autoGenerated += self.uiMap.findAutoGenerated(commands)
            recordScripts.append((script, commands))
        if len(autoGenerated) > 0:
            autoGeneratedInfo = self.uiMap.parseAutoGenerated(autoGenerated)
            self.resetInstrumentation(testMode)
            newNames = DomainNameGUI(autoGeneratedInfo, self).collectNames()
            self.uiMap.storeNames(zip(autoGeneratedInfo, newNames))
            replacements = zip(autoGenerated, newNames)
            for script, commands in recordScripts:
                newCommands = [ self.makeReplacement(c, replacements) for c in commands ]
...
class DomainNameGUI:
    def __init__(self, autoGenerated, scriptEngine):
        ...
        self.allEntries = []
        win.set_name("Name Entry Window")
        win.set_title("Enter Usecase names for auto-recorded actions")
        win.connect("delete_event", self.quit)
        scriptEngine.monitorSignal("close name entry window", "delete_event", win, autoGenerated=True)
    ...
    def collectNames(self):
        gtk.main()
        return [ entry.get_text() for entry in self.allEntries ]
