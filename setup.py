...
setup(name='PyUseCase',
      ...
      description="An unconvential GUI-testing tool for UIs written with PyGTK, Tkinter, wxPython, SWT or Eclipse RCP",
      long_description='PyUseCase is an unconventional GUI testing tool, with support for PyGTK, Tkinter, wxPython, SWT and Eclipse RCP. Instead of recording GUI mechanics directly, it asks the user for descriptive names and hence builds up a "domain language" along with a "UI map file" that translates it into the current GUI layout. The point is to reduce coupling, allow very expressive tests, and ensure that GUI changes mean changing the UI map file but not all the tests. Instead of an "assertion" mechanism, it auto-generates a log of the GUI appearance and changes to it. The point is then to use that as a baseline for text-based testing, using e.g. TextTest. It also includes support for instrumenting code so that "waits" can be recorded, making it far easier for a tester to record correctly synchronized tests without having to explicitly plan for this.',
      packages=["usecase", "usecase.gtktoolkit", "usecase.gtktoolkit.simulator", "usecase.gtktoolkit.describer",
                "usecase.javaswttoolkit", "usecase.javarcptoolkit", "usecase.javaswingtoolkit" ],
      package_dir={ "" : "lib"},
